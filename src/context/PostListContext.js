...
export const PostListProvider = props => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // console.log('resultPosts in context', resultPosts)
      // For random numbers see 
			//   https://stackoverflow.com/questions/4959975/generate-random-number-between-two-numbers-in-javascript
      const thePosts = resultPosts.data.slice(0, 8).map( (post,i) => { return { ...post, category: categories[Math.floor(Math.random() * 4)], postDate: dates[i] }})
